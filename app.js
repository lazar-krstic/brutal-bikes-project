...
app.post('/servis1', function(req, res) {
    var data = {
...
fs.writeFile(__dirname + "/db_data/service.json", jsonData, function(err){
    if(err) throw err;
}
);
    res.redirect("/home")
...
var jsonData = JSON.stringify(data)
fs.writeFile(__dirname + "/db_data/contact.json", jsonData, function(err){
    if(err) throw err;
}
);
    res.redirect("/home")
